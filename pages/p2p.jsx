...
export default function P2P() {
  ...
  const getOrders = async()=>{
    ...
    for(let i =0; i<orders[0].length; i++){
      or.push(<Transaction appr ={approveUSD} address={orders[0][i]} key={i} orderData={orders[1][i]} connectedAd={connectedAccount} pur={buy} placed={orders[1][i]['orderPlaced']}/>)
    }
    console.log(or);
    changeAllOrders(or);
  ...
  const withWeb3 = async()=>{
    ...
    console.log(o)
    var myOrder=<Transaction address={conAccount[0]} orderData={o} connectedAd={conAccount[0]} cancelO={cancelOrder} placed={o['orderPlaced']}/>
    console.log(myOrder);
  ...
  const placeOrder=async()=>{
    ...
    await contract.methods.placeAnOrder(nodeSaleNum.current.value,String(nodeSalePrice.current.value*10**6)).send({"from":connectedAccount[0]});
    var o =await contract.methods.orders(connectedAccount[0]).call();
    console.log(o)
    var myOrder=<Transaction address={connectedAccount[0]} orderData={o} connectedAd={connectedAccount[0]} cancelO={cancelOrder} placed={o['orderPlaced']}/>
    console.log(myOrder);
    updateMyOrders(myOrder);
    getOrders();
  }
  ...
  return (
    <div className={styles.p2p}>
      <div className={styles.overview + " row"}>
        ...
        <div className="col-xl-6">
         {(!connectedAccount && <button className="connect" onClick={withWeb3}>Connect Wallet</button>)}
          <div className={styles.avatar}>
            <img src="/assets/Magus5.png" alt="" />
          </div>
          <div className={styles.info}>
            {(connectedAccount && <><p>Connected to:</p><p>{connectedAccount}</p></>)}
        ...
        <Tabs defaultActiveKey="buy" id="uncontrolled-tab-example" className="mb-3">
          ...
          <Tab eventKey="sell" title="Sell">
            <Table responsive="lg">
              ...
              <tbody>
                <><tr className={stylesT.row}>
      <td>
        <div>
          <div className={stylesT.avatar}>
